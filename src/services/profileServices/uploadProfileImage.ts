...
export const uploadImageAndGetURL = async (image: File, folder: string): Promise<string | null> => {
    try {
      ...
      if (data) {
        const { data: publicURL } = await supabase.storage.from('media').getPublicUrl(data.path);
